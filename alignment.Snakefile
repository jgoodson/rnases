...
rule regressive_tcoffee_align:
    ...
    threads: 4
    shell:
        "t_coffee -reg -seq {input:q} -reg_method {wildcards.mode}.tc_method -reg_tree mbed -reg_nseq 100 -thread={threads} -outfile {wildcards.file}.tcr.{wildcards.mode}.aln -email {config[email]} -run_name `mktemp -d`/run"
