...
rule regressive_tcoffee_align:
    ...
    output:
        "{file}.tcr.{method}.aln"
    shell:
        "t_coffee -reg -seq {input:q} -reg_method {wildcards.method}.tc_method -reg_tree mbed -reg_nseq 100 -outfile {wildcards.file}.tcr.{wildcards.method}.aln -email {config[email]} -run_name `mktemp -d`/run"
